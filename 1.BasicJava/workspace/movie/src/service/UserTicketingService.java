...
public class UserTicketingService {
	...
	SimpleDateFormat sDate = new SimpleDateFormat("Y년 M월 d일 aa h시:m분");
	SimpleDateFormat ssDate = new SimpleDateFormat("Y년 M월 d일");
	...
	*/
	...
	public int userticketing() {
		System.out.println("╭─────────────────────────────╮");
		System.out.println("|  1.상영중인 영화 2.예매하기 3.홈으로     |");
		System.out.println("╰─────────────────────────────╯");
	...
	public int usermovieinfo() { //상영중인 영화
		...
		System.out.println("자세히보고 싶은 영화번호 :             0.돌아가기");
		int movieno = ScanUtil.nextInt();
		if(movieno == 0) {
			return View.USER_TIKETING;
		}
		Map<String,Object> details = userticketingdao.moviedetails(movieno);
		...
		System.out.print("■ 상영기간:"+ssDate.format(details.get("MOVIE_TERM1"))+" ~ " + ssDate.format(details.get("MOVIE_TERM2")));
		System.out.print("\t■ 출연진:"+details.get("MOVIE_ACTER")+"\n");
		System.out.print("■ 줄거리:"+details.get("MOVIE_SUMMARY")+"\n");
		
		System.out.println("1.리뷰보기 2.예매하러가기 3.이전으로");
		int input = ScanUtil.nextInt();
		switch(input) {
		case 1 :
			List<Map<String,Object>> rvinfo = userticketingdao.reviewinfo(movieno);
			for(int i = 0; i < rvinfo.size(); i++) {
			Map<String,Object> rvlist = rvinfo.get(i);	
			System.out.print(rvlist.get("M_NAME") + "\t");
			System.out.print(rvlist.get("REVIEW_CONTENT") + "\t");
			System.out.println(rvlist.get("REVIEW_SCORE"));
			}
	...
	public int ticketing() {
		...
		if(movielist.size() != 0) {
			...
			System.out.print("번호 "+mmap.get("MOVIE_NO")+"\t");	
			System.out.print("제목 "+mmap.get("MOVIE_NAME")+"\n");
			}
			}else {
				System.out.println("없음");
			}
			System.out.println("영화 번호 선택 :             0.돌아가기");
			int movieno = ScanUtil.nextInt();
			if(movieno == 0) {
				return View.USER_TIKETING;
			}
			...
			for(int i = 0; i < rtimelist.size(); i++) {
				...
				System.out.print("시작시간: "+sDate.format(timelist.get("TIME_START"))+"\t");
				System.out.print("종료시간: "+sDate.format(timelist.get("TIME_END"))+"\t");
				System.out.print("영화제목: "+timelist.get("MOVIE_NAME")+"\t");
				System.out.print("상영관번호: "+timelist.get("SC_NO")+"\n");
				System.out.println("──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────");
			}
			
			System.out.println("예약할 상영번호 :           0.돌아가기");
	...
	public int ticketinfo(int timeno) {
		...
		*/
		...
		System.out.println("상영시간 : " + sDate.format(info.get("TIME_START")) + " ~ " + sDate.format(info.get("TIME_END")));
		System.out.println("\n─────────────────────────────────────");
		...
		case 1:
			...
			System.out.println("기본관람료 : " + result);
			if(seat.size()>1) {
				for(int i = 2; i <= seat.size(); i++)
				result += result;
			}
			...
			if(agesale.size() != 0) {
				...
				System.out.println(agesale.get("P_NAME")+": "+agesale.get("P_COST"));
			}
			...
			if(seatsale != null) {
				...
				System.out.println("좌석요금 : " + seatsale.get("SEAT_CHARGE"));
			}
		    System.out.println("───────────────────────────────────");
			System.out.println("최종결제 가격 : " + result);
