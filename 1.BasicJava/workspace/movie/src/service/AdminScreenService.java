...
public class AdminScreenService {
	...
	public int AdminScreen() {
	      System.out.println("╭────────────────────────────────────────────────╮");
	      System.out.println("|1.상영관설정  2.관람료설정  3.상영스케줄설정  4.관리자 홈 0.로그아웃   ");
	      System.out.println("╰────────────────────────────────────────────────╯");
	...
	public int screenSet() {
		...
		*/
		...
		for (int i = 0; i < screenlist.size(); i++) {
			...
			System.out.print("총 좌석 :"+screen.get("SEAT_ALL") + "\n");
			System.out.println("─────────────────────────────────────");
		}
	...
	public int screenPlus() { //상영관추가하기
		System.out.println("추가할 상영관 이름 :         0.이전으로");
	...
	public int seatSet() { //좌석
		System.out.print("상영관 이름 :          0.돌아가기");
		String scname = ScanUtil.nextLine();
		if(scname.equals("0")) {
			return View.SCREEN_SET;
		}
		...
		System.out.println();
		System.out.println("요금 설정할 라인 입력 : ex)A    0.돌아가기");
		String seatname = ScanUtil.nextLine();
		if(seatname.equals("0")) {
			return View.SCREEN_SET;
		}
	...
	*/
	...
	public int timetableSet() {
		...
		System.out.println();
		System.out.println("설정할 상영관 이름 :          0.돌아가기");
		String scname = ScanUtil.nextLine();
		if(scname.equals("0")) {
			return View.SCREEN_ADMIN;
		}
		...
		for(int i = 0; i < rtimelist.size(); i++) {
			...
			System.out.print("상영시간 : "+sDate.format(list.get("TIME_START"))+" ~ " + sDate.format(list.get("TIME_END")) +" 영화이름 : " + list.get("MOVIE_NAME")+"\n");
		}
		System.out.println("──────────────────────────────────────────────────────────────────────");
		System.out.println("1.회차추가하기 2.회차삭제하기  3.이전으로");
		...
		case 1 : //회차추가하기 누르면..
			...
			System.out.println("──────────────────── 상영가능 영화목록 ────────────────────");
			//영화목록
			List<Map<String, Object>> mlist =adminscreendao.selectMovieList();
			if(mlist.size() != 0) {
			for(int i = 0; i < mlist.size(); i++) {
			Map<String,Object> mmap = mlist.get(i);
			System.out.print("영화 번호 :"+mmap.get("MOVIE_NO")+"  ");	
			System.out.print("영화 제목 :"+mmap.get("MOVIE_NAME")+"\t");
			System.out.print("러닝 타임 :"+mmap.get("MOVIE_TIME")+"\n");
			}
			}else {
