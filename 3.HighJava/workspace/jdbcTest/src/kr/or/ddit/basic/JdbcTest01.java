	...
	*/
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 3-3) SQL문을 서버로 전송해서 실행하고, 실행한 결과를 얻어온다.
			...
			// 포인터의 시작은 0 레코드 부터 시작한다고 생각하면 편하다 == > rs.next() 호출시 1번으로 이동하기때문에 true 반환
			while(rs.next()) {
				// 현재 rs객체의 포인터가 가리키는 곳의 자료를 가져오는 법
				// 형식1) rs.get자료형이름("컬럼명"); ==> 컬럼명은 대소문자 구분없이 사용가능
				// 형식2) rs.get자료형이름(컬럼번호); ==> 컬럼번호는 1부터 시작...
				// 형식3) rs.get자료형이름("컬럼의 alias명); ==> 컬럼번호는 1부터 시작.
